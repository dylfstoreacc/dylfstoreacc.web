...
export default function Home() {
  ...
  const fetchUlasan = async () => {
    try {
      ...
      if (!error && data) setDaftarUlasan(data)
    } catch (err) {
  ...
  const handleSubmitUlasan = async (e) => {
    ...
    if (result.success) {
      setStatusMsg('Ulasan terkirim! Menunggu saringan admin Dileppp 🌱')
      e.target.reset()
      fetchUlasan()
    } else {
      setStatusMsg('Gagal mengirim ulasan.')
    }
  }
  ...
  return (
    <>
      ...
      {showSplash && (
        <div style={styles.splashScreen}>
          <div style={styles.loaderWrapper}>
            <div style={styles.glowRing} className="ring-rotate"></div>
            <img src="/dfstoreacc.png" style={styles.loadLogo} className="logo-pulse" alt="DYLF Logo" onError={(e) => { e.target.src = 'https://placehold.co/115x115/05050a/00ffff?text=DYLF' }} />
          </div>
        </div>
      )}

      {/* 2. POP-UP MODAL ABOUT STORE */}
      {showAbout && (
        <div style={styles.modalOverlay}>
          <div style={styles.modalCard}>
            ...
            <div style={styles.modalBodyText}>
              Kini sistem kami terintegrasi penuh dengan Next.js + Supabase Database Cloud di tahun 2026, diproteksi ketat oleh <strong>Difibot</strong> demi kenyamanan transaksi konsumen secara real-time.
            </div>
      ...
      {/* 3. BINGKAI GAWAI HANDPHONE UTAMA (MENGUNCI PRESISI) */}
      <div style={styles.phoneContainer}>
        <div style={styles.scrollContent} className="custom-scroll">
          
          <div style={styles.storeBanner}>
            <img src="/banner.png" style={styles.bannerImg} alt="" onError={(e) => { e.target.style.display = 'none' }} />
          </div>
          ...
          <div style={styles.mainContentWrapper}>
            ...
            {activePage === 'home' && (
              <div>
                <div style={styles.profileContainer}>
                  <img src="/dfstoreacc.png" style={styles.profileLogo} alt="DYLF Logo" onError={(e) => { e.target.src = 'https://placehold.co/170x170/05050a/00ffff?text=DYLF' }} />
                ...
                <div style={styles.sosmedInstructionText} className="text-pulse">🎯 Click Icon to Contact Us 🎯</div>
                
                {/* BARIS IKON SOSMED MENYALA SEJAJAR PRESISI */}
                <div style={styles.sosmedRow}>
                  <a href="https://wa.me/6285266953530" target="_blank" rel="noopener noreferrer" style={styles.sosmedIcon} className="neon-border-glow"><i className="fa-brands fa-whatsapp" style={{color: '#25D366'}}></i></a>
                  <a href="https://www.tiktok.com/@dylfrvr" target="_blank" rel="noopener noreferrer" style={styles.sosmedIcon} className="neon-border-glow"><i className="fa-brands fa-tiktok" style={{color: '#ffffff'}}></i></a>
                  <a href="https://instagram.com/dylfrvr" target="_blank" rel="noopener noreferrer" style={styles.sosmedIcon} className="neon-border-glow"><i className="fa-brands fa-instagram" style={{color: '#f9376c'}}></i></a>
                </div>

                <div style={styles.waLinksWrapper}>
                  <div style={styles.mascotAbsoluteContainer}>
                    <img src="/images/difibot_v2.png" style={styles.mascotImg} alt="Difibot" onError={(e) => { e.target.src = 'https://placehold.co/195x195/080b16/bc13fe?text=Difibot' }} />
                  </div>
                  
                  <a href="https://chat.whatsapp.com/IyHj3XJikuX90k6HSiMpDP" target="_blank" rel="noopener noreferrer" style={styles.groupCard}>
                    <div>
                      <h3 style={{color:'#fff', fontSize:'13px', margin:0}}>GB 1: DYLF STOREacc</h3>
                      <p style={{color:'#a0aec0', fontSize:'10px', margin:0}}>Grup komunitas utama pusat jual & beli akun aman</p>
                    </div>
                    <div style={{color:'#00ffff', fontWeight:'bold'}}>&gt;</div>
                  </a>
                  <a href="https://whatsapp.com/channel/0029VbCmpXQ8vd1RqxIn6h0z" target="_blank" rel="noopener noreferrer" style={styles.groupCard}>
                    <div>
                      <h3 style={{color:'#fff', fontSize:'13px', margin:0}}>CH: DYLF STOREacc</h3>
                      <p style={{color:'#a0aec0', fontSize:'10px', margin:0}}>Channel update info stok akun & track history testi</p>
                    </div>
                    <div style={{color:'#00ffff', fontWeight:'bold'}}>&gt;</div>
                  </a>
                </div>
            ...
            {/* HALAMAN 2: TESTIMONI FOTO */}
            {activePage === 'testi' && (
              <div style={styles.testiSectionWrapper}>
                <div style={{textAlign:'center', marginBottom:'15px'}}>
                  <h2 style={{fontSize:'18px', color:'#00ffff', textTransform:'uppercase', margin:0}}>Track History</h2>
                  <p style={{fontSize:'11px', color:'#a0aec0', margin:0}}>Bukti transaksi sukses bersama DYLF STOREacc</p>
                </div>
                ...
                <div style={styles.testiGrid}>
                  <p style={styles.noTestiText}>Silakan unggah aset screenshot bukti transaksi kamu ke folder public/testi/stok dan rekber di GitHub.</p>
                </div>
              </div>
            )}

            {/* HALAMAN 3: ULASAN SUPABASE */}
            {activePage === 'ulasan' && (
              <div>
                <div style={styles.reviewStatsSummary}>
                  <div style={styles.bigRatingNumber}>5.0 / 5.0</div>
                  {/* TOTAL BINTANG SUMMARY SEJAJAR CYAN GLOWING */}
                ...
                <div style={styles.reviewScrollContainer} className="custom-scroll">
                  {daftarUlasan.length > 0 ? (
                    daftarUlasan.map((item) => (
                      <div key={item.id} style={styles.standaloneReviewCard}>
                        ...
                        <p style={{fontSize:'12px', color:'#e2e8f0', lineHeight:'1.45', margin:0}}>{item.text}</p>
                      </div>
                    ))
                  ) : (
                ...
                <div style={styles.formReviewBox}>
                  ...
                  <form onSubmit={handleSubmitUlasan}>
                    ...
                    <div style={{marginBottom:'12px'}}>
                      ...
                      <select name="rating" style={{...styles.inputField, color:'#00ffff', fontWeight:'bold', background:'#080b16'}}>
                  ...
                  {statusMsg && <p style={{fontSize:'11px', color:'#00ffff', textAlign:'center', marginTop:'10px', margin:0}}>{statusMsg}</p>}
                </div>
              </div>
            )}
...
// INLINE EMULASI MODEL UKURAN PRESISI (ANTI LUBER & FULL GLOW RAPI)
const styles = {
  ...
  // KUNCI UTAMA BINGKAI LAYAR HP (DIJAMIN DI TENGAH PAS)
  phoneContainer: { width: '100vw', maxWidth: '430px', height: '100vh', maxHeight: '880px', backgroundColor: '#05050a', border: '2px solid #bc13fe', boxShadow: '0 0 35px rgba(188, 19, 254, 0.4), inset 0 0 20px rgba(0, 255, 255, 0.15)', borderRadius: '24px', display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden' },
  scrollContent: { flex: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch', overflowX: 'hidden' },
  
  storeBanner: { width: '100%', height: '130px', background: 'linear-gradient(45deg, #0a2463 0%, #050508 100%)', borderBottom: '1px solid rgba(0, 255, 255, 0.3)', display: 'flex', justifyContent: 'center', alignItems: 'center' },
  ...
  profileLogo: { width: '160px', height: '160px', objectFit: 'contain', marginBottom: '2px', filter: 'drop-shadow(0 0 12px rgba(0, 255, 255, 0.5))' },
  ...
  bioBox: { background: 'rgba(10, 36, 99, 0.4)', border: '1px solid #bc13fe', borderRadius: '14px', padding: '14px', fontSize: '13px', lineHeight: '1.5', color: '#e2e8f0', marginBottom: '15px' },
  
  sosmedInstructionText: { textAlign: 'center', fontSize: '10px', fontWeight: '700', color: '#00ffff', textTransform: 'uppercase', letterSpacing: '2px', marginBottom: '15px' },
  
  // BARIS ICON SOSMED FIX FLUID RAPI DI TENGAH
  sosmedRow: { display: 'flex', justifyContent: 'center', gap: '25px', marginBottom: '25px', position: 'relative', zIndex: 10 },
  sosmedIcon: { width: '48px', height: '48px', borderRadius: '50px', display: 'flex', justifyContent: 'center', alignItems: 'center', fontSize: '22px', backgroundColor: '#050508', border: '2px solid transparent', textDecoration: 'none', cursor: 'pointer' },
  
  waLinksWrapper: { display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '12px', position: 'relative', marginTop: '15px' },
  mascotAbsoluteContainer: { position: 'absolute', left: '-22px', bottom: '-12px', width: '190px', height: '190px', zIndex: 3, pointerEvents: 'none' },
  mascotImg: { width: '100%', height: '100%', objectFit: 'contain', filter: 'drop-shadow(0 0 10px #00ffff)' },
  groupCard: { background: 'rgba(10, 36, 99, 0.4)', borderLeft: '4px solid #bc13fe', borderTop: '1px solid rgba(188, 19, 254, 0.2)', borderBottom: '1px solid rgba(188, 19, 254, 0.2)', borderRight: '1px solid rgba(188, 19, 254, 0.2)', borderRadius: '0 14px 14px 0', padding: '12px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', textDecoration: 'none', marginLeft: '135px' },
  ...
  reviewStatsSummary: { backgroundColor: 'rgba(0, 255, 255, 0.04)', border: '1px dashed rgba(0, 255, 255, 0.3)', borderRadius: '16px', padding: '15px', textAlign: 'center', marginTop: '15px', marginBottom: '20px' },
  bigRatingNumber: { fontSize: '32px', fontWeight: '900', color: '#00ffff', textShadow: '0 0 10px rgba(0, 255, 255, 0.4)' },
  bigRatingStars: { color: '#00ffff', fontSize: '13px', margin: '4px 0', letterSpacing: '2px', textShadow: '0 0 6px #00ffff' },
  reviewScrollContainer: { display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '25px', maxHeight: '340px', overflowY: 'auto' },
  standaloneReviewCard: { backgroundColor: 'rgba(10, 36, 99, 0.3)', border: '1px solid rgba(188, 19, 254, 0.15)', borderRadius: '14px', padding: '14px' },
  reviewItemStars: { color: '#00ffff', fontSize: '11px', marginBottom: '6px', textShadow: '0 0 5px #00ffff', display: 'flex', gap: '3px' },
  
  formReviewBox: { backgroundColor: 'rgba(13, 13, 20, 0.9)', border: '1px solid #bc13fe', borderRadius: '18px', padding: '18px', boxShadow: '0 0 15px rgba(188,19,254,0.15)' },
  formBoxTitle: { fontSize: '14px', fontWeight: '800', color: '#00ffff', textTransform: 'uppercase', marginBottom: '14px', textShadow: '0 0 6px #00ffff' },
  ...
  submitReviewBtn: { width: '100%', background: 'linear-gradient(90deg, #bc13fe, #0a2463)', color: '#ffffff', border: 'none', borderRadius: '10px', padding: '12px', fontSize: '11px', fontWeight: '700', letterSpacing: '1px', cursor: 'pointer', textTransform: 'uppercase', boxShadow: '0 0 10px rgba(188, 19, 254, 0.4)' },
  
  footerPrivacy: { textAlign: 'center', marginTop: '35px', paddingTop: '15px', borderTop: '1px solid rgba(255, 255, 255, 0.05)', display: 'flex', justifycontent: 'center', alignItems: 'center', gap: '15px', justifyContent: 'center' },
